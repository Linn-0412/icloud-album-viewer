...
const { applyInferredDateCards } = require('./timeline');
const APPLE_HEADERS = {
...
class ICloudAlbumError extends Error {
...
function parsePublicToken(input) {
...
async function postAppleJson(url, payload) {
  const retryCount = Math.max(0, Number(process.env.ICLOUD_REQUEST_RETRIES ?? DEFAULT_ICLOUD_RETRIES));
  let lastError = null;

  for (let attempt = 0; attempt <= retryCount; attempt += 1) {
    try {
      return await postAppleJsonOnce(url, payload);
    } catch (error) {
      lastError = error;
...
async function postAppleJsonOnce(url, payload) {
  let response;
  ...
  try {
    response = await fetch(url, {
      ...
      redirect: 'manual',
      signal: AbortSignal.timeout(Number(process.env.ICLOUD_REQUEST_TIMEOUT_MS || DEFAULT_ICLOUD_TIMEOUT_MS))
    });
  } catch (error) {
...
function isRetryableICloudError(error) {
  return (
    ...
    RETRYABLE_HTTP_STATUSES.has(Number(error?.appleStatusCode))
  );
}
...
async function fetchWebstream(baseUrl, token) {
  const firstResponse = await postAppleJson(`${baseUrl}webstream`, { streamCtag: null });
  ...
  const redirectedResponse = await postAppleJson(`${redirectedBaseUrl}webstream`, { streamCtag: null });
...
async function fetchAssetUrls(baseUrl, photoGuids) {
  ...
  const batches = [];
  const batchDelayMs = Math.max(0, Number(process.env.ICLOUD_ASSET_BATCH_DELAY_MS ?? DEFAULT_ASSET_BATCH_DELAY_MS));

  for (let index = 0; index < photoGuids.length; index += ASSET_BATCH_SIZE) {
    batches.push(photoGuids.slice(index, index + ASSET_BATCH_SIZE));
  }

  await runWithConcurrency(batches, Number(process.env.ICLOUD_ASSET_CONCURRENCY || DEFAULT_ASSET_CONCURRENCY), async (batch) => {
    const response = await postAppleJson(`${baseUrl}webasseturls`, { photoGuids: batch });
...
function normalizePhoto(photo, index, urlsByChecksum) {
...
function applyAssetUrlsToPhoto(photo, urlsByChecksum) {
...
async function fetchSharedAlbumMetadata(input) {
  const token = parsePublicToken(input);
  const initialBaseUrl = getBaseUrl(token);
  const webstream = await fetchWebstream(initialBaseUrl, token);
...
async function fetchSharedAlbumAssets(baseUrl, photos, photoIds) {
  ...
  const urlsByChecksum = await fetchAssetUrls(baseUrl, photoGuids);
...
async function fetchSharedAlbum(input) {
  const album = await fetchSharedAlbumMetadata(input);
  const photoIds = album.photos.map((photo) => photo.id);

  return {
    metadata: album.metadata,
    photos: await fetchSharedAlbumAssets(album.baseUrl, album.photos, photoIds)
  };
}
...
module.exports = {
  ...
  parseDateInfo,
  normalizePhoto,
  applyAssetUrlsToPhoto,
  sortPhotos,
  fetchSharedAlbumMetadata,
